...
func init() {
	...
	_, err := TypedSchema.Indexes().AddIndexByColTags(IndexName, []uint64{NameTag}, types.Format_Default, schema.IndexProperties{IsUnique: false, Comment: ""})
	if err != nil {
		panic(err)
	}
	_, err = UntypedSchema.Indexes().AddIndexByColTags(IndexName, []uint64{NameTag}, types.Format_Default, schema.IndexProperties{IsUnique: false, Comment: ""})
	if err != nil {
		panic(err)
	}
