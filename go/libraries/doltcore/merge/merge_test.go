...
func init() {
	indexSchema, _ = sch.Indexes().AddIndexByColTags("idx_col1", []uint64{col1Tag}, types.Format_Default, schema.IndexProperties{IsUnique: false, Comment: ""})
	compositeIndexSchema, _ = sch.Indexes().AddIndexByColTags("idx_col1_col2", []uint64{col1Tag, col2Tag}, types.Format_Default, schema.IndexProperties{IsUnique: false, Comment: ""})
}
