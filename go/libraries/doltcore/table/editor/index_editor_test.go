...
func TestIndexEditorConcurrency(t *testing.T) {
	...
	require.NoError(t, err)
	index, err := tableSch.Indexes().AddIndexByColNames("idx_concurrency", []string{"v1"}, format, schema.IndexProperties{IsUnique: false, Comment: ""})
	require.NoError(t, err)
...
func TestIndexEditorUniqueMultipleNil(t *testing.T) {
	...
	tableSch, err := schema.SchemaFromCols(colColl)
	require.NoError(t, err)
	index, err := tableSch.Indexes().AddIndexByColNames("idx_unique", []string{"v1"}, format, schema.IndexProperties{IsUnique: true, Comment: ""})
	require.NoError(t, err)
	indexSch := index.Schema()
	emptyMap, err := types.NewMap(context.Background(), vrw)
	require.NoError(t, err)
...
func TestIndexEditorUniqueErrorDoesntPersist(t *testing.T) {
	...
	tableSch, err := schema.SchemaFromCols(colColl)
	require.NoError(t, err)
	index, err := tableSch.Indexes().AddIndexByColNames("idx_unq", []string{"v1"}, format, schema.IndexProperties{IsUnique: true, Comment: ""})
	require.NoError(t, err)
	indexSch := index.Schema()
	emptyMap, err := types.NewMap(context.Background(), vrw)
	require.NoError(t, err)
...
func TestIndexRebuildingUniqueSuccessOneCol(t *testing.T) {
	...
	index, err := sch.Indexes().AddIndexByColTags("idx_v1", []uint64{2}, types.Format_Default, schema.IndexProperties{IsUnique: true, Comment: ""})
	require.NoError(t, err)
...
func TestIndexRebuildingUniqueSuccessTwoCol(t *testing.T) {
	...
	index, err := sch.Indexes().AddIndexByColTags("idx_v1", []uint64{2, 3}, types.Format_Default, schema.IndexProperties{IsUnique: true, Comment: ""})
	require.NoError(t, err)
	updatedTable, err := originalTable.UpdateSchema(context.Background(), sch)
	require.NoError(t, err)
...
func TestIndexEditorCapacityExceeded(t *testing.T) {
	...
	tableSch, err := schema.SchemaFromCols(colColl)
	require.NoError(t, err)
	index, err := tableSch.Indexes().AddIndexByColNames("idx_cap", []string{"v1"}, types.Format_Default, schema.IndexProperties{IsUnique: false, Comment: ""})
	require.NoError(t, err)
	indexSch := index.Schema()
