...
func TestKeylessTableEditorMultipleIndexErrorHandling(t *testing.T) {
	...
	tableSch, err := schema.SchemaFromCols(colColl)
	require.NoError(t, err)
	idxv1, err := tableSch.Indexes().AddIndexByColNames("idx_v1", []string{"v1"}, format, schema.IndexProperties{
		IsUnique: false,
	})
	require.NoError(t, err)
	idxv2, err := tableSch.Indexes().AddIndexByColNames("idx_v2", []string{"v2"}, format, schema.IndexProperties{
		IsUnique: false,
	})
	require.NoError(t, err)
