...
func getRemoteCommitTracking(ctx context.Context, dEnv *env.DoltEnv) error {
	...
	// TODO : fetch is needed to update remote branch ref hash. This will fai, if there is commit added in remote branch
	...
	var msg string
	// TODO : 1 commit vs 2 commits
	if ahead > 0 && behind > 0 {
		msg = fmt.Sprintf("Your branch and '%s' have diverged,\nand have %v and %v different commits each, respectively. ", rbn, ahead, behind)
		msg += fmt.Sprintf("\n  (use \"dolt pull\" to update your local branch)")
	} else if ahead > 0 {
		msg = fmt.Sprintf("Your branch is ahead of '%s' by %v commits.", rbn, ahead)
		msg += fmt.Sprintf("\n  (use \"dolt push\" to publish your local commits)")
	} else if behind > 0 {
		msg = fmt.Sprintf("Your branch is behind '%s' by %v commits, and can be fast-forwarded.", rbn, behind)
		msg += fmt.Sprintf("\n  (use \"dolt pull\" to update your local branch)")
	} else {
		msg = fmt.Sprintf("Your branch is up to date with '%s'.", rbn)
	}
